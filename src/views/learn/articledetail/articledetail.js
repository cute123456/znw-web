...
export default {
    ...
    methods: {
        goMoreArticle(id) {
            this.$router.push('understand?id=' + id)
        },
        shareBtn() {
        ...
        // 获取文章详情
        getDetail() {
            this.axios.get('/wechatauth/study/article/detail', {
                params: {
            ...
            setTimeout(() => {
                ...
                this.$wechat.ready(() => {
                    ...
                    this.$wechat.onMenuShareAppMessage({ // 分享给朋友
                        ...
                        // 用户确认分享后执行的回调函数
                        success: function() {
                            // that.$dialog.toast({ mes: '分享文章成功！', timeout: 3000 });
                            that.axios.post('/wechatauth/benefit/share', { id: that.$route.query.id, type: 2 }).then(res => {
                                console.log(res)
                                if (res.data.result) {
