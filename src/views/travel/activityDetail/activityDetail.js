    ...
    methods: {
        ...
        getActivityDetail() {
            this.axios.get('/wechatauth/travel/activity/detail', {
                ...
                this.activityDetail.detail = this.activityDetail.detail.replace(/<img/g, '<img style="max-width:100%;height:auto" ') //防止富文本图片过大
                this.activityDetail.detail = this.activityDetail.detail.replace(/<iframe/g, '<iframe style="max-width:100%;height:auto" ')
                        ...
                        this.axios.get('/wechatauth/user/info').then(res => {
                            if (res !== undefined && res.data.result) {
                                if (!res.data.datas.mobile) {
                                    ...
                                    this.$router.push('/travel/activitySignup?id=' + this.$route.query.id)
                                }
                            } else {
                                this.$router.push('/travel/activitySignup?id=' + this.$route.query.id)
                            }
