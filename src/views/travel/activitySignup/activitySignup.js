...
export default {
    ...
    methods: {
        ...
        // 获取公益详情
        getActivityDetail() {
            this.axios.get('/wechatauth/travel/activity/detail', {
                params: {
        ...
        //立即报名
        toApply() {
            if (this.applyInfo.member_name === "" || this.applyInfo.gender === "" || this.applyInfo.mobile === "") {
                this.$dialog.toast({ mes: '请将您的信息填写完整~', timeout: 3000 });
                return;
            } else {
                if (this.applyInfo.integral === "") {
                    this.applyInfo.integral = 0
                } else if (this.applyInfo.integral > this.memberInfo.integral) {
                    this.$dialog.toast({ mes: '您的积分不足！', timeout: 3000 });
                    return;
                } else if (this.applyInfo.integral > this.activityDetail.maxNumber) {
                    this.$dialog.toast({ mes: '输入积分大于可兑换积分~', timeout: 3000 });
                    return;
                }
        ...
        // 发起微信支付
        wxPay() {
            this.axios.post('/wechatauth/travel/activity/enlist', {
                activity_id: this.$route.query.id,
                real_name: this.applyInfo.member_name,
                gender: this.applyInfo.gender,
                mobile: this.applyInfo.mobile,
                address: this.applyInfo.address,
                wechat: this.applyInfo.wechat,
                deductible_integral: this.applyInfo.integral
            }).then(res => {
                console.log(res)
            ...
            WeixinJSBridge.invoke(
                ...
                function(res) {
                    ...
                    if (res.err_msg == "get_brand_wcpay_request:ok") { //如果微信支付成功
                        ...
                        that.axios.post('/wechatauth/travel/activity/confirm/pay', {
                            ordersn: that.ordersn
                        }).then(result => {
                            if (result.data.result) {
                                that.$dialog.toast({ mes: '支付成功！', timeout: 3000 });
                                that.$router.push('/member/travelRecords')
                            } else {
